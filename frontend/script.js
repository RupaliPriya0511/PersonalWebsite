...
async function addProject() {
    ...
    const deleteKey = prompt("Enter the secret key to submit this project:");
    if (deleteKey !== "Rupali@0511") {
    alert("Incorrect key! Project not submitted.");
    return;
  }
    let title = document.getElementById("title").value;
    let date = document.getElementById("date").value;
...
function addProjectToUI(project) {
    ...
    projectCard.innerHTML = `
        <h3>${project.title}</h3>
        <p><strong>Completed On:</strong> ${formatDate(project.date)}</p>
